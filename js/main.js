...
async function fetchWeather(city) {
  ...
  const snowRaw =
    cur.snowfall != null ? cur.snowfall : hourlyVal(hourly && hourly.snowfall);
...
function buildSceneElements(scene, type, isNight) {
  ...
  if (type === "clear" && !isNight) {
    ...
    sun.innerHTML = '<div class="sun-core"></div><div class="sun-rays"></div>';
...
async function reverseGeocode(lat, lon) {
  try {
    ...
    if (data.results && data.results.length) {
      ...
      const id = (r.name.toLowerCase() + " " + (r.country_code || "x")).replace(
        /[^a-z0-9]/g,
        "",
      );
      return {
...
async function loadRainViewerData() {
  try {
    ...
    if (!satelliteFrames.length) {
      console.warn("Keine Satellitenbilder verfügbar");
    }

    radarCurrentPos = Math.max(0, radarFrames.length - 1);
  } catch (err) {
    console.warn("RainViewer API Fehler:", err);
    // Fallback: Leere Arrays damit die App nicht crasht
    radarFrames = [];
    satelliteFrames = [];
  }
}
...
function showSatelliteFrame(pos) {
  ...
  Object.values(satelliteLayers).forEach(
    (layer) => layer && radarMap.hasLayer(layer) && radarMap.removeLayer(layer),
  );
...
function renderMinutely(w) {
  ...
  const horizon = 5;
  const slots = [];
  for (let i = idx; i < Math.min(idx + horizon, m.precipitation.length); i++) {
    slots.push({
      t: m.time[i],
...
function renderTempChartWeek(w, container) {
  ...
  const yAt = (v) => padTop + (1 - (v - lo) / span) * (H - padTop - padBottom);
...
function applyLanguage() {
  ...
  // Sektionstitel
  const favTitle = document.querySelector("#favorites-section .section-title");
