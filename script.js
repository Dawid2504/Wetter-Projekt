document.addEventListener("DOMContentLoaded", () => {
  ...
  const VAPID_PUBLIC_KEY =
    "BGpPbqFrqGCpVLNv-035yicDT1TXW_pSWM_pwg23TnyjPA8FOZwgBF6VrvFGnPWlmfBAi6nAHeROITsxI1Jcxis";
  ...
  async function enablePush() {
    ...
    const subJson = sub.toJSON(); // { endpoint, keys: { p256dh, auth } }
    const db = window.__db;
    const { collection, addDoc, query, where, getDocs } = window.__fs;

    // Duplikat-Check per Endpoint
    const col = collection(db, "subscriptions");
    const existing = await getDocs(
      query(col, where("endpoint", "==", subJson.endpoint)),
    );
    if (!existing.empty) {
      console.log("Abo existiert bereits.");
      return;
    }
